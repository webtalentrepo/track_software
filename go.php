...
<?php } else { 
	...
	<script type="text/javascript">
		...
		function loadFrame(f) {
			if (started) {
				loadedFrames++;
				if (loadedFrames>totalFrames) location.href = '<?php echo $link; ?>';
				else {
	...
	foreach ($lists as $lid=>$l) {
			$outHtml .= str_replace(Array('{list_id}','{email}','{url}','{title}'),Array($l['list_id'],$_GET['email'],$l['url'],$l['title']),$l['template'])."\r\n"; 
